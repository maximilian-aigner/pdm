...
function(fit1, maxit, pmax, nvars, vnames) {
  ...
  dd <- c(nvars, nalam)
  #cluster <- matrix(fit1$cluster[seq(nvars * nalam)], nvars,nalam)[seq(nbetamax), 
                                                            #  , drop = FALSE]
  if (nbetamax > 0) {
    ...
    beta <- new("dgCMatrix", Dim = dd, Dimnames = list(vnames, 
                                                       stepnames), x = as.vector(beta[oja, ]), p = as.integer(ibeta - 
                                                                                                                1), i = as.integer(ja - 1))
 # cluster<- new("dgCMatrix", Dim = dd, Dimnames = list(vnames, 
       #                                                stepnames), x = as.vector(cluster[oja, ]), p = as.integer(ibeta - 
                                                                                                            #    1), i = as.integer(ja - 1))
  } else {
 ...
 cluster<-matrix(fit1$cluster,nvars,nalam)
  ...
  list(b0 = b0, beta = beta, df = df, dim = dd, lambda = lam ,cluster=t(cluster))
}
################
lambda.interp <-
function(lambda, s) {
