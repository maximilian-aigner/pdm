...
normc = function(X,center=T) {
  ...
  zero.cols <- apply(X.centered == 0, 2, all)
  scaling.factor <- sqrt(colSums(X.centered^2))
  scaling.factor[zero.cols] <- 1
  X.scaled = scale(X.centered, center=F, scale=scaling.factor)
